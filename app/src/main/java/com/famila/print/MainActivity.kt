...
class MainActivity : Activity() {
    ...
    private fun loadPairedPrinters() {
        ...
        if (!adapter.isEnabled) {
            setStatus("Bluetooth kapalı. Önce Bluetooth'u açın.")
            return
        }

        printers.clear()
        printers.addAll(adapter.bondedDevices.sortedBy { it.name ?: it.address })
        ...
        if (printers.isEmpty()) {
            ...
            setStatus("XP-P328B'yi önce Android Bluetooth ayarlarından eşleştirin.")
            return
        }

        val labels = printers.map {
            val name = it.name?.takeIf { n -> n.isNotBlank() } ?: "Bluetooth cihazı"
            "$name  •  ${it.address}"
        }
        printerSpinner.adapter = ArrayAdapter(
            this,
    ...
    private fun printLabel() {
        if (printers.isEmpty()) {
            Toast.makeText(this, "Önce XP-P328B'yi Bluetooth ile eşleştirin.", Toast.LENGTH_LONG).show()
            return
        }
        ...
        if (width == null || width !in 10.0..80.0) {
            widthInput.error = "10–80 mm arasında bir genişlik girin"
            return
        }
        if (height == null || height !in 10.0..80.0) {
            heightInput.error = "10–80 mm arasında bir yükseklik girin"
            return
        }
        if (count == null || count !in 1..100) {
            countInput.error = "1–100 arasında adet girin"
            return
        }
        if (barcode.isBlank()) {
            barcodeInput.error = "Barkod boş olamaz"
            return
        }

        val index = printerSpinner.selectedItemPosition.coerceIn(0, printers.lastIndex)
        val device = printers[index]

        prefs.edit()
            .putString("width", width.toString().removeSuffix(".0"))
            .putString("height", height.toString().removeSuffix(".0"))
            .putString("count", count.toString())
            .putString("printer_mac", device.address)
            .apply()

        printButton.isEnabled = false
        setStatus("${device.name ?: "Yazıcı"} cihazına bağlanılıyor…")
        ...
        Thread {
            try {
                ...
                runOnUiThread {
                    ...
                    setStatus("✅ $count etiket yazıcıya gönderildi.")
                    Toast.makeText(this, "Etiket gönderildi", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                runOnUiThread {
                    printButton.isEnabled = true
                    setStatus("❌ Yazdırma başarısız: ${e.message ?: e.javaClass.simpleName}")
                }
            }
        }.start()
    }
    ...
    private fun buildTspl(
        ...
        val dotsPerMm = 8
        val hDots = (heightMm * dotsPerMm).toInt()
        val compact = heightMm < 20
        ...
        if (compact) {
            ...
            val barcodeY = 26
            val barcodeHeight = (hDots - barcodeY - 18).coerceIn(34, 62)
            ...
            val barcodeY = 70
            val barcodeHeight = (hDots - barcodeY - 28).coerceIn(55, 105)
