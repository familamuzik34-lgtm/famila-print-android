...
class MainActivity : Activity() {
    ...
    private fun generateNextEan13() {
        ...
        val next = nextPayload + ean13CheckDigit(nextPayload)
    ...
    private fun ean13CheckDigit(first12: String): Int {
        var sum = 0
        first12.forEachIndexed { index, ch -> sum += if (index % 2 == 0) ch.digitToInt() else ch.digitToInt() * 3 }
        return (10 - (sum % 10)) % 10
    }

    private fun isValidEan13(value: String): Boolean =
        value.length == 13 && value.all { it.isDigit() } && ean13CheckDigit(value.take(12)) == value.last().digitToInt()

    private fun hasBluetoothPermissions(): Boolean = Build.VERSION.SDK_INT < Build.VERSION_CODES.S || checkSelfPermission(Manifest.permission.BLUETOOTH_CONNECT) == PackageManager.PERMISSION_GRANTED
    private fun requestBluetoothPermissions() { if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) requestPermissions(arrayOf(Manifest.permission.BLUETOOTH_CONNECT), 1001) }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        if (requestCode == 1001) { if (hasBluetoothPermissions()) loadPairedPrinters() else setStatus("Bluetooth bağlantı izni gerekli.") }
    }
    ...
    private fun loadPairedPrinters() {
        ...
        if (printers.isEmpty()) { printerSpinner.adapter = ArrayAdapter(this, android.R.layout.simple_spinner_dropdown_item, listOf("Eşleşmiş yazıcı yok")); return }
    ...
    private fun printLabel() {
        ...
        val type = typeInput.text.toString().trim(); val brand = brandInput.text.toString().trim(); val color = colorInput.text.toString().trim(); val product = productInput.text.toString().trim(); val barcode = barcodeInput.text.toString().trim()
        ...
        if (barcode.length == 13 && barcode.all { it.isDigit() } && !isValidEan13(barcode)) { barcodeInput.error = "EAN-13 kontrol hanesi hatalı"; return }
        ...
        printButton.isEnabled = false; setStatus("${d.name ?: "Yazıcı"} cihazına bağlanılıyor…")
    ...
    private fun sendTspl(d: BluetoothDevice, w: Double, h: Double, c: Int, type: String, brand: String, color: String, product: String, barcode: String) {
        val bytes = buildTspl(w,h,c,type,brand,color,product,barcode).toByteArray(Charset.forName("windows-1254")); var last:Exception?=null
        val tries=listOf<(BluetoothDevice)->BluetoothSocket>({it.createInsecureRfcommSocketToServiceRecord(sppUuid)},{it.createRfcommSocketToServiceRecord(sppUuid)},{x->x.javaClass.getMethod("createRfcommSocket",Int::class.javaPrimitiveType).invoke(x,1) as BluetoothSocket})
        for(f in tries){var s:BluetoothSocket?=null;try{s=f(d);s.connect();Thread.sleep(200);s.outputStream.apply{write(bytes);flush()};Thread.sleep(300);return}catch(e:Exception){last=e}finally{try{s?.close()}catch(_:Exception){}}};throw Exception(last?.message?:"SPP kanal hatası")
    }
    ...
    private fun buildTspl(w: Double, h: Double, count: Int, type: String, brand: String, color: String, product: String, barcode: String): String {
        ...
        val dots=(w*8).toInt(); val sb=StringBuilder("SIZE ${fmt(w)} mm,${fmt(h)} mm\r\nGAP 2 mm,0 mm\r\nDIRECTION 1\r\nREFERENCE 0,0\r\nCLS\r\nCODEPAGE 1254\r\n")
        fun textWidth(s:String,font:Int)=s.length*(if(font==3)24 else if(font==2)16 else 12)
        fun centerX(s:String,font:Int)=((dots-textWidth(s,font))/2).coerceAtLeast(6)
        fun addText(y:Int,s:String,font:Int){if(s.isNotBlank())sb.append("TEXT ${centerX(s,font)},$y,\"$font\",0,1,1,\"${safe(s)}\"\r\n")}
        val ean=isValidEan13(barcode)
        when {
            h < 15 || w < 32 -> { addText(3,type,2); val bw=if(ean)210 else 220; val bx=((dots-bw)/2).coerceAtLeast(5); sb.append("BARCODE $bx,23,\"${if(ean) "EAN13" else "128"}\",40,1,0,1,2,\"${safe(barcode)}\"\r\n") }
            h < 24 || w < 38 -> { addText(4,type,2); addText(27,brand,1); val bw=if(ean)230 else 240; val bx=((dots-bw)/2).coerceAtLeast(5); sb.append("BARCODE $bx,48,\"${if(ean) "EAN13" else "128"}\",52,1,0,1,2,\"${safe(barcode)}\"\r\n") }
            else -> {
                val typeFont=if(type.length>20)1 else 2; val brandFont=if(brand.length>23)1 else 2
                addText(7,type,typeFont); addText(31,brand,brandFont)
                val detail=listOf(color,product).filter{it.isNotBlank()}.joinToString(" "); addText(55,detail,1)
                val bw=if(ean)230 else 250; val bx=((dots-bw)/2).coerceAtLeast(6); val bh=((h*8).toInt()-132).coerceIn(48,62)
                sb.append("BARCODE $bx,77,\"${if(ean) "EAN13" else "128"}\",$bh,1,0,1,2,\"${safe(barcode)}\"\r\n")
            }
        }
